...
class BoardWidget extends Pad {
    ...
    /** Handle a mouse-button push on S. NEED FIX. */
    private void mousePressed(Square s) {
        if (_movingSquare == null
                && _board.get(s) == _board.turn()) {
            _movingSquare = s;
        } else if (s.col() == _movingSquare.col()
                && s.row() == _movingSquare.row()) {
            _movingSquare = null;
        } else {
            String sqbegin1 = _movingSquare.toString().substring(0, 1);
            String sqend1 = _movingSquare.toString().substring(1);
            String sqbegin2 = s.toString().substring(0, 1);
            String sqend2 = s.toString().substring(1);

            _commands.offer(sqbegin1 + sqend1 + "-" + sqbegin2 + sqend2);
            _movingSquare = null;
        }
        repaint();
    }

    /** Handle a mouse-button release on S. NEED FIX. */
    ...
    /** Saves the last square that is clicked. */
    private Square _movingSquare = null;
