...
class CornersProblem(search.SearchProblem):
    ...
    def isGoalState(self, state):
        ...
        return (current in corners) and (len(corners) == 1)
    ...
    def getNextState(self, state, action):
        ...
        current, corners = state
        return ((nextx, nexty), corners if current not in corners else tuple([i for i in corners if i != current]))
...
def foodHeuristic(state, problem):
    ...
    for item in list(foodGrid.asList()):
        foodX, foodY = item
        disX = foodX - x
        disY = foodY - y
