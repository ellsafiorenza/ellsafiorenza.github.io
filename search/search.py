...
def aStarSearch(problem, heuristic=nullHeuristic):
    ...
    while not priorityQ.isEmpty():
        ...
        if state not in visitedNode:
            visitedNode.append(state)
            for successor, action, stepCost in problem.expand(state):
                priorityQ.update((successor, path + [action], cost + stepCost), cost + stepCost + heuristic(successor, problem))
